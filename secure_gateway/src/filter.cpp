...
int Filter::add_iptables_rule(Filter::rule rule){
    ...
    if (pid == 0)
    {
        char* argument_list[] = {command,add_chain_flag, chain, source_ip_flag, src_ip_local, dst_ip_flag, dst_ip_local, proto_flag, proto_local, source_port_flag, src_port_local, dst_port_flag, dst_port_local, action_flag, rule.action,NULL};
...
int Filter::delete_iptables_rule(Filter::rule rule){
    char src_ip_local[32];
    char dst_ip_local[32];
    char src_port_local[8];
    char dst_port_local[8];
    ...
    if (pid == 0)
    {
        char* argument_list[] = {command,delete_chain_flag, chain, source_ip_flag, src_ip_local, dst_ip_flag, dst_ip_local, proto_flag, proto_local, source_port_flag, src_port_local, dst_port_flag, dst_port_local, action_flag, rule.action,NULL};
        if(execvp("/usr/sbin/iptables",argument_list) == -1){
            exit(-1);
        }
    }else{
